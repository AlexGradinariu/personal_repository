...
def insert_IMX_serial(location_of_imx8_tool,serial_number):
    if len(serial_number.strip()) != 20:
        print("Lenght of serial number looks incorrect !")
        print("Script will stop here !")
        sys.exit()
    with open(os.path.join(location_of_imx8_tool,"00_input\TCU_Serial_Numbers.txt"), "w") as file:
        file.write(serial_number.strip())
...
def create_folder_and_cp_imx_samfile(location_of_imx8_tool):
    sam_folder_name = os.path.join(os.path.dirname(location_of_imx8_tool),sn)
    try:
        os.mkdir(sam_folder_name)
    except FileExistsError as e:
        print(e)
        print(f"Folder {sn} already existing,script will stop here...!")
        sys.exit(1)
    try :
...
def modify_debug_policy_serial(new_serial,location_of_imx8_tool):
    ...
    try:
        with open(os.path.join(os.path.dirname(location_of_imx8_tool), ("sdx24_debugpolicy.xml" if sn[4] == "E" else "sdx55_debugpolicy.xml")), "r") as file:
        ...
        with open(os.path.join(os.path.dirname(location_of_imx8_tool), ("sdx24_debugpolicy.xml" if sn[4] == "E" else "sdx55_debugpolicy.xml")), "w") as file1:
            for line in lines:
                file1.write(line)
        print("Entry debug policy was modified !" if sn[4] == "E" else "HIGH debug policy was modified !")
    except Exception as e :
        print(f"Exception occured as {e} !")
        sys.exit(1)
...
def copy_to_linuxmachine_modified_file(location_of_imx8_tool):
    ...
    local_file = os.path.join(os.path.dirname(location_of_imx8_tool), ("sdx24_debugpolicy.xml" if sn[4] == "E" else "sdx55_debugpolicy.xml"))
    '''Make a difference between HIGH and ENTRY, by checking the serial number apended in above list os serial'''
    if sn[4] == "E":
        with SCPClient(ssh.get_transport()) as scp:
            try:
...
def copy_debug_policy_file(location_of_imx8_tool):
    ...
    if sn[4] == "E":
        with SCPClient(ssh.get_transport()) as scp:
            try:
                scp.get(entry_sdx_file, os.path.join(os.path.dirname(location_of_imx8_tool), sn), recursive=True)
        ...
        with SCPClient(ssh.get_transport()) as scp:
            try:
                scp.get(high_sdx_file, os.path.join(os.path.dirname(location_of_imx8_tool), sn), recursive=True)
                print("Your HIGH debug policies files are succesfully copied !")
            except Exception as e:
                print(f"Exception occured as {e}!")

def create_debug_policy_archive(location_of_imx8_tool):
    if os.path.exists(os.path.join(os.path.dirname(location_of_imx8_tool), sn)):
        print(f"Creating archive for folder : {sn} !")
        shutil.make_archive(os.path.join(os.path.dirname(location_of_imx8_tool), sn), "zip", os.path.join(os.path.dirname(location_of_imx8_tool), sn))
    else:
        print(f"Folder : {sn} not existing,stopping here...")
        sys.exit(1)


if __name__ == "__main__":
    lista_seriale = [x for x in open("Serial_numbers.txt", "r")]
    #create ssh client
    ssh = createSSHClient()
    #create docker container and install prerequisites
    try:
        (stdin, stdout, stderr) = ssh.exec_command(
            "docker run -id --privileged --rm --name self-service-NAD --network host -v $HOME:$HOME -v /PROJ:/PROJ -v /etc/resolv.conf:/etc/resolv.conf -v /dev:/dev -v /sys/kernel/security:/sys/kernel/security vni-ce-devops-center-docker-l.artifactory.geo.conti.de/ubuntu_dev:18.04_drt15_prod /bin/bash")
        print(stdout.read().decode())
        print(stderr.read().decode())
        time.sleep(5)
        _, stdout, _ = ssh.exec_command(
            "docker exec -i -u ${USER} --workdir /home/uidn1902/Downloads/NAD_Debug_Policy_Tool/Downloads self-service-NAD sudo pip install pyOpenSSL")
        print(stdout.read().decode())
        _, stdout, _ = ssh.exec_command(
            "docker exec -i -u ${USER} --workdir /home/uidn1902/Downloads/NAD_Debug_Policy_Tool/Downloads self-service-NAD sudo pip install requests_toolbelt")
        print(stdout.read().decode())
        _, stdout, _ = ssh.exec_command(
            "docker exec -i -u ${USER} --workdir /home/uidn1902/Downloads/NAD_Debug_Policy_Tool/Downloads self-service-NAD sudo chmod -R +x ./")
        print(stdout.read().decode())
    except Exception as e :
        print(e)
    for sn in lista_seriale:
        sn = sn.strip("\n")
        insert_IMX_serial(imx_dbgplc_folder,sn)
