...
# ---------------- Git Operations ----------------
def clone_gerrit_repo(project, branch, commit_hash=None,user=None):
    current_dir = os.getcwd()
    user = 'uic81314'
    server = 'buic-scm-ias.automotive-wan.com'
    port = '29418'
    ssh_url = f"ssh://{user}@{server}:{port}/{project}"
    ...
    scp_command = [
        "scp", "-p", "-O", "-P", port,
        f"{user}@{server}:hooks/commit-msg",
        f"{local_path}/.git/hooks"
    ]

    try:
        logger.info(f"Cloning {project}@{branch}...")
        ...
        logger.info("Copying commit-msg hook...")
...
def push_changes_to_github(github_repo, gerrit_repo_name, github_branch, commit_hash=None, gerrit_repo_branch=None):
    ...
    remote_name = 'github'
    try:
        print(f'git remote add {remote_name} {github_repo}')
        # Add remote if not exists, else update
        ...
        logger.info(f"Pushing changes to GitHub ({refspec})...")
